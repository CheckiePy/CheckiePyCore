    ...
    def count(self, file, verbose=False):
        with open(file) as f:
            ...
            for name, line in names:
                # Underscored or camel case with underscore (i.e. "other")
                if '_' in name:
                    # If underscored name contains big letters or multiple neighboring underscores
                    if any(ch.isupper() for ch in name) or name.startswith('_') or name.endswith('_') or ('__' in name):
                        other_count += 1
